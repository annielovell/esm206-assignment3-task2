...
The name "Ki-Adi-Mundi" has two dashes so when the columns were split at the first dash, the third part of the name "-Mundi" was dropped.

```{r}
# Your code here: 

sw_split_name <- starwars %>% # Start with starwars dataset and use assignment operator to name the new dataset "sw_split_name"
...
sw_skin_sep <- starwars %>% # Start from starwars data to use assignment operator to create new data set called sw_skin_sep
  separate(col = skin_color, # Separate the `skin_color` column
           into = c("skin_1","skin_2"), # Into two columns named skin_1 and skin_2
           sep = ",", # Creating the separation at the first *comma*
           extra = "merge", # Merge everything after the first comma into second column
           remove = FALSE) # And *do not* remove the original column
```
...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "VROOM!" # Puts "HEY!" between the pieces (default is underscore)
  )
...
some_dates <- c(11101985, 12101999, 01042020) # Creates a vector of dates w/o spaces
some_dates_converted <- mdy(my_dates) # Converts to Date, stores as some_dates_converted
...
gasol_hookshots <- lakers %>% #Start with the lakers dataset and use assignment operator to name the new dataset "gasol_hookshots
  filter(player == "Pau Gasol") %>% #Filer lakers dataset to only include player "Pau Gasol"
  filter(type == "hook") #Filer lakers dataset to only include shot type "hook"
  
#Take the "gasol_hookshots" dataset and using ymd() - year month day format - changed the data to "Date" class:

gasol_hookshots <- gasol_hookshots %>% 
  mutate(date = ymd(date))  # Converts to date

class(gasol_hookshots$date) #Check the class of date --> the class should report back as date

gasol_hookshots <- gasol_hookshots %>% 
  mutate(month_new = month(date))  #Creates a new column for month

# Find the *number* of Gasol's hook shots recorded (not necessarily scored) by **month** (hint: `count(month)`) for the 2008 - 2009 season

gasol_hook_shots <- gasol_hookshots %>% 
  group_by(month_new) %>% 
  count(month_new) %>% 
  summarize(gasol_hook_shots = sum(month_new))
...
storms_danger <- storms %>% # Start from storms data
  mutate(danger = case_when( # Add a new column danger, which contains... 
    status == "hurricane" ~ "high", # "high" if status equals 'hurricane'
    status != "hurricane" ~ "moderate")) # "moderate" if status equals something other than hurricane
```
